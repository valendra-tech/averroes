...
impl Tool for ListSkillsTool {
    ...
    fn description(&self) -> &str { "List all available skills with their names and descriptions" }
    ...
    async fn execute(&self, _ctx: &ToolContext, _params: &serde_json::Value) -> Result<ToolResult> {
        ...
        let output: Vec<String> = skills.iter().map(|s| format!("- **{}**: {}", s.name, s.description)).collect();
        if output.is_empty() {
