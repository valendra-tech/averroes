...
impl WorkspaceStore {
    ...
    pub fn active_workspace(&self) -> Option<&WorkspaceConfig> {
        self.index
            .active
            .as_ref()
            .and_then(|id| self.index.workspaces.iter().find(|w| &w.id == id))
    }
    ...
    fn last_reasoning_effort_path() -> PathBuf {
        dirs::home_dir()
            .unwrap_or_else(|| PathBuf::from("."))
            .join(".config")
            .join("averroes")
            .join("last_reasoning_effort.txt")
    }

    pub fn save_last_reasoning_effort(effort: &str) {
        let path = Self::last_reasoning_effort_path();
        if let Some(parent) = path.parent() {
            let _ = std::fs::create_dir_all(parent);
        }
        let _ = std::fs::write(&path, effort);
    }

    pub fn load_last_reasoning_effort() -> Option<String> {
        let path = Self::last_reasoning_effort_path();
        let content = std::fs::read_to_string(&path).ok()?;
        let effort = content.trim().to_string();
        if effort.is_empty() {
            None
        } else {
            Some(effort)
